    ...
    <div class="outcome-kind-row">
      <label class="field-label">
        ...
        <select
          ...
          @change="onKindChange(($event.target as HTMLSelectElement).value as OutcomeEffect['kind'])"
        >
          <option v-for="opt in EFFECT_KIND_OPTIONS" :key="opt.id" :value="opt.id">{{ opt.label }}</option>
        </select>
      </label>
    </div>
...
const props = defineProps<{
  modelValue: OutcomeEffect
  refOptions?: Partial<AdminRefOptions>
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: OutcomeEffect): void
}>()

const draft = computed(() => props.modelValue)
const meta = computed(() => OUTCOME_EFFECT_META[props.modelValue.kind])

function onKindChange(kind: OutcomeEffect['kind']) {
  emit('update:modelValue', makeDefaultEffect(kind))
}

function setField(key: string, value: unknown) {
  emit('update:modelValue', { ...props.modelValue, [key]: value } as OutcomeEffect)
}

function allowCustomRef(field: FieldDef): boolean {
  return field.refType === 'flags' || field.refType === 'counters' || field.refType === 'materials'
}
